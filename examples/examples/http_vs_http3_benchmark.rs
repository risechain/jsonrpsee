...
async fn main() -> anyhow::Result<()> {
	...
	for _ in 0..args.connections {
		let client = if args.protocol == "http3" {
			#[cfg(feature = "http3")]
			{
				...
				let client = builder.enable_http3().build(&server_url).await?;
	...
	let requests_per_second = total_requests as f64 / total_elapsed.as_secs_f64();

	println!("\n=== Benchmark Results ===");
	println!("Protocol:           {}", args.protocol);
	println!("Payload size:       {} elements", args.payload_size);
	println!("Total requests:     {}", total_requests);
	println!("Successful:         {} ({}%)", successful, (successful as f64 / total_requests as f64) * 100.0);
